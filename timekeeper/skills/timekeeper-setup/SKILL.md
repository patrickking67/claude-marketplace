...
1. **Connectors** (see `${CLAUDE_PLUGIN_ROOT}/CONNECTORS.md`):
   - **Microsoft 365 (required, read — Claude's native connector):** confirm it's connected and can see mail/calendar/Teams. Smoke-test by listing a couple of recent calendar events and reporting what you see. If it's missing, walk them through enabling Claude's built-in M365 connector — nothing works without it.
   - **Zoom / Webex / Calendly (optional, Claude built-ins):** mention these as available extra signal for meeting-heavy timekeepers. Don't push — only flag if the user says they use one and it's not connected.
